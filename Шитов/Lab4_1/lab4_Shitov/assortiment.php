    ...
    <div class="table_assorti">
        <center>
            ...
            <form style="padding-top: 20px" method="post">
                ...
                <select name="filter">
                    ...
                    <optgroup label="По Типу" value="Тип">
                        ...
                        include "vendor/connect.php";
                        $expo = array();
                        $query_sel = "SELECT distinct Наименование from Тип";
            ...
            if(isset($_POST["filtrate"]))
            {
                ...
                {
                    $filter_type = "";
                    $req = "SELECT *, Тип.Наименование FROM Ассортимент INNER JOIN Тип on (Ассортимент.ID_Типа = Тип.ID_Типа)";
                    print_store($req);
                } else {
                    include "vendor/connect.php";
                    $filter_type = "WHERE Тип.Наименование LIKE '" . $filter . "'";
                    $req = "SELECT *, Тип.Наименование FROM Ассортимент INNER JOIN Тип on (Ассортимент.ID_Типа = Тип.ID_Типа)" . $filter_type;
                    print_store($req);
                }
            } else {
                $req = "SELECT *, Тип.Наименование FROM Ассортимент INNER JOIN Тип on (Ассортимент.ID_Типа = Тип.ID_Типа)";
...
function print_store($req)
{
    ...
    if ($req) {
        echo "<center><table class='table'><tr><th>Наименование продукта</th><th>Тип</th><th>Цена (Р)</th><th>Заказать</th></tr>";
        while ($row_u = mysqli_fetch_assoc($req)) {
            echo "<tr><td>". $row_u['Наименование_вып']."</td><td>".$row_u['Наименование']."</td>
                    <td>".$row_u['Цена']."</td><td><input type=\"checkbox\" name=\"choices[]\" value = \"" . $row_u['ID_продукта'] . "\"></td>
                    </tr>";
        }
        echo "</table></center>
