...
--
-- Хост: localhost:8889
-- Время создания: Апр 06 2023 г., 14:08
...
INSERT INTO `user` (`id_user`, `ФИО_пользователя`, `email`, `Логин`, `Пароль`, `Дата_регистрации`) VALUES
(1, 'ваываыавыв', 'mablokochanelgame@gmail.com', 'aakoval', '1234', '2023-04-06 12:35:52'),
(2, 'Насыров Владислав', 'nasirov@gmail.com', 'nasirov', '1234', '2023-04-06 13:56:53');
...
CREATE TABLE `Команды` (
  ...
  `Логотип_команды` varchar(100) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `Команды` (`ID_Команды`, `Название`, `Кол_побед`, `Кол_поражений`, `ID_Конференции`, `Позиция_в_конф`, `Позиция_в_лиге`, `Штат`, `Логотип_команды`) VALUES
(1, 'Милуоки Бакс', 34, 25, 1, 3, 7, 'Милуоки, Висконсин', 'assets/img/1.jpeg'),
(2, 'Финикс Санз', 35, 13, 2, 1, 1, 'Финикс, Аризона', 'assets/img/2.jpeg'),
(3, 'Мемфис Гриззлиз', 45, 22, 2, 3, 3, 'Мемфис, Теннесси', 'assets/img/3.jpeg'),
(4, 'Голден Стэйт Уорриорз', 43, 21, 2, 2, 2, 'Сан-Франциско, Калифорния', 'assets/img/4.jpeg'),
(5, 'Майами Хит', 43, 22, 1, 1, 4, 'Майами, Флорида', 'assets/img/5.jpeg'),
(6, 'Юта Джаз', 40, 23, 2, 4, 5, 'Солт-Лэйк_Сити, Юта', 'assets/img/6.jpeg'),
(7, 'Филадельфия', 39, 24, 1, 2, 6, 'Филадельфия, Пенсильвания', 'assets/img/7.jpeg'),
(8, 'Чикаго Буллз', 33, 25, 1, 4, 8, 'Чикаго, Иллинойс', 'assets/img/8.jpeg'),
(9, 'Лос-Анджелес Лейкерс', 28, 35, 2, 9, 20, 'Калифорния', 'assets/img/9.jpeg');
...
INSERT INTO `Корзина` (`id_заказа`, `id_пользователя`, `id_мерча`, `Дата_заказа`) VALUES
(1, 1, 1, '2023-04-06 12:47:22'),
(2, 1, 1, '2023-04-06 13:19:16'),
(3, 2, 1, '2023-04-06 13:59:40'),
(4, 2, 2, '2023-04-06 13:59:40');
...
CREATE TABLE `Мерч` (
  `id_мерча` int(7) NOT NULL,
  `Название_мерча` varchar(100) NOT NULL,
  `Тип` varchar(100) NOT NULL,
  `id_команды` int(7) DEFAULT NULL,
...
INSERT INTO `Мерч` (`id_мерча`, `Название_мерча`, `Тип`, `id_команды`, `Стоимость`) VALUES
(1, 'Игровая футболка', 'футболка', 4, '10000.00'),
(2, 'Игровые шорты', 'шорты', 4, '10000.00'),
(3, 'Баскетбольный мяч финала 2016 года', 'Мяч', 4, '25000.00');
...
ALTER TABLE `Корзина`
  ...
  ADD KEY `корзина_ibfk_2` (`id_мерча`);
...
--
ALTER TABLE `Игроки`
  MODIFY `ID_Игрока` int(7) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;
...
--
ALTER TABLE `Конференция`
  MODIFY `ID_конф` int(7) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
--
ALTER TABLE `Медиа`
  MODIFY `ID_Медиа` int(7) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
...
--
ALTER TABLE `Корзина`
  ADD CONSTRAINT `корзина_ibfk_1` FOREIGN KEY (`id_пользователя`) REFERENCES `user` (`id_user`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `корзина_ibfk_2` FOREIGN KEY (`id_мерча`) REFERENCES `Мерч` (`id_мерча`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `Мерч`
  ADD CONSTRAINT `мерч_ibfk_1` FOREIGN KEY (`id_команды`) REFERENCES `Команды` (`ID_Команды`) ON DELETE CASCADE ON UPDATE CASCADE;
