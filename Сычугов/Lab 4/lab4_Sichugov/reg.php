...
function reg($login, $name, $email, $phone, $pass){
    ...
    elseif(!checkSize($pass, 1, 18))
        ...
        include "vendor/connect.php";
        $quer = 'SELECT COUNT(*) FROM Пользователь WHERE `Логин` LIKE \''.$login.'\'' ;
        ...
        if($row[0] > 0)
        {
            ...
            $insert = "INSERT INTO `Пользователь` (`id_пользователя`, `ФИО_польз`, `email`, `телефон`, `Логин`, `Пароль`) VALUES (NULL, '$name', '$email', '$phone', '$login', '$pass')";
