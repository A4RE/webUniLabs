    ...
    <div class="table_merch">
        <center>
            ...
            <form method="post">
                ...
                <select name="filter">
                    ...
                    <optgroup label="По Типу" value="Тип">
                        ...
                        include "vendor/connect.php";
                        $expo = array();
                        $query_sel = "SELECT distinct Тип_мерча from мерч";
            ...
            if(isset($_POST["filtrate"]))
            {
                ...
                {
                    $filter_type = "";
                    $req = "SELECT *, Команда.Название_команды FROM мерч INNER JOIN Команда on (Мерч.id_команды = Команда.ID_Команды)";
                    print_store($req);
                } else {
                    include "vendor/connect.php";
                    $filter_type = "WHERE мерч.Тип_мерча LIKE '" . $filter . "'";
                    $req = "SELECT *, Команда.Название_команды FROM мерч INNER JOIN Команда on (Мерч.id_команды = Команда.ID_Команды)" . $filter_type;
                    print_store($req);
                }
            } else {
                $req = "SELECT *, Команда.Название_команды FROM мерч INNER JOIN Команда on (Мерч.id_команды = Команда.ID_Команды)";
...
function print_store($req)
{
    ...
    if ($req) {
        ...
        while ($row_u = mysqli_fetch_assoc($req)) {
            echo "<tr><td>". $row_u['Название_мерч']."</td>
                    <td>".$row_u['Тип_мерча']."</td><td>".$row_u['Название_команды']."</td><td>".$row_u['Цена']."</td><td><input type=\"checkbox\" name=\"choices[]\" value = \"" . $row_u['id_мерча'] . "\"></td>
                    </tr>";
        }
        echo "</table></center>
