...
<div class="container">
    <div class="table_merch">
        <center>
            <h5 align="center">Выберите Зоопарк</h5>
            <form method="post">
                <select name="filter">
                    ...
                    <optgroup label="По Типу" value="Тип">
                        ...
                        include "vendor/connect.php";
                        $expo = array();
                        $query_sel = "SELECT distinct Назв_зпарка from Зоопарк";
            ...
            if(isset($_POST["filtrate"]))
            {
                ...
                {
                    $filter_type = "";
                    $req = "SELECT *, Зоопарк.Назв_зпарка FROM Билет INNER JOIN Зоопарк on (Билет.id_зоопарка = Зоопарк.ID_зоопарка)";
                    print_store($req);
                } else {
                    include "vendor/connect.php";
                    $filter_type = "WHERE Зоопарк.Назв_зпарка LIKE '" . $filter . "'";
                    $req = "SELECT *, Зоопарк.Назв_зпарка FROM Билет INNER JOIN Зоопарк on (Билет.id_зоопарка = Зоопарк.ID_зоопарка)" . $filter_type;
                    print_store($req);
                }
            } else {
                $req = "SELECT *, Зоопарк.Назв_зпарка FROM Билет INNER JOIN Зоопарк on (Билет.id_зоопарка = Зоопарк.ID_зоопарка)";
...
function print_store($req)
{
    ...
    if ($req) {
        echo "<center><table class='table'><tr><th>Название зоопарка</th><th>Цена Билета (Р)</th><th>Заказать</th></tr>";
        while ($row_u = mysqli_fetch_assoc($req)) {
            echo "<tr><td>". $row_u['Назв_зпарка']."</td>
                    <td>".$row_u['Цена']."</td><td><input type=\"checkbox\" name=\"choices[]\" value = \"" . $row_u['id_билета'] . "\"></td>
                    </tr>";
        }
        echo "</table></center>
