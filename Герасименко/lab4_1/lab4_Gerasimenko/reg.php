...
function reg($login, $name, $email, $phone, $pass){
    ...
    elseif(!checkSize($pass, 1, 18))
        ...
        include "vendor/connect.php";
        $quer = 'SELECT COUNT(*) FROM Посетитель WHERE `Логин` LIKE \''.$login.'\'' ;
        ...
        if($row[0] > 0)
        {
            ...
            $insert = "INSERT INTO `Посетитель` (`id_посетителя`, `ФИО_посетителя`, `email`, `телефон`, `Логин`, `Пароль`, `дата_регистрации`) VALUES (NULL, '$name', '$email', '$phone', '$login', '$pass', CURRENT_TIMESTAMP)";
